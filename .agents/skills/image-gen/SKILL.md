...
```bash
node .Codex/skills/image-gen/scripts/generate-image.mjs -n <name> [options] <prompt>
```
...
```bash
node .Codex/skills/image-gen/scripts/upload-image.mjs -n [npc-name-lowercase] images/generated/<file>.png
```
...
```bash
node .Codex/skills/image-gen/scripts/cleanup-variants.mjs -n [name] -k [picked-number]
```
