...
1. **Map the subsystem.** Identify every file involved in the workflow that failed:
   ...
   - AGENTS.md if the orchestrator itself was involved
...
- **Agent** (`.Codex/agents/*.md`) — delegation, chaining, or agent behavior was wrong
- **Reference** (`references/*.md`) — schema documentation was incorrect or incomplete
- **Rule** (`.Codex/rules/*.md`) — orchestrator's own behavior was wrong
- **Script** (`.Codex/scripts/*.js`) — hook script has a bug or missing logic
- **AGENTS.md** — orchestrator's core instructions need adjustment
